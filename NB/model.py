...
def my_init():
    ...
    # 读入stopwords
    with open("d:\\Users\\CN096\\实验\\NLP\\实践作业1代码框架\\NB_framework\\stopwords.txt", encoding="utf-8") as f:
    ...
    for i in range(len(neg_sents_train)):
        st = jieba.lcut(neg_sents_train[i])  # 分词，返回词列表
        st = remove_stopwords(st)  # 去掉停用词
        neg_words += st
        

    # 获得负向训练语料的词频表_neg_dict
    for i in set(neg_words):
        _neg_dict[i]=neg_words.count(i)
        
    sum_words_neg = len(set(neg_words))

    # 获得正向训练语料的词列表pos_words
    for i in range(len(pos_sents_train)):
        st = jieba.lcut(pos_sents_train[i])  # 分词，返回词列表
        st = remove_stopwords(st)  # 去掉停用词
        pos_words += st
        

    # 获得正向训练语料的词频表_pos_dict
    for i in set(pos_words):
        _pos_dict[i]=pos_words.count(i)
    
    sum_words_pos=len(set(neg_words))
...
if __name__ == "__main__":
    ...
    print(sum_words_neg, sum_words_pos)
    print(len(neg_sents_train), len(pos_sents_train))
    p0=len(neg_sents_train)/(len(neg_sents_train)+len(pos_sents_train))
    p1=1-p0
    print(p0,p1)
    ...
    for i in range(len(neg_sents_test)):  # 用negative的句子做测试
        ...
        """
        for j in st:
            if j in neg_dict_keys:
                p_neg += np.log(neg_dict[j] / sum_words_neg)
            else:
                p_neg += np.log(1 / sum_words_neg)
            if j in pos_dict_keys:
                p_pos += np.log(pos_dict[j] / sum_words_pos)
            else:
               p_pos += np.log(1/sum_words_pos)

        if p_pos+np.log(p1) < p_neg+np.log(p0):
            rights += 1
